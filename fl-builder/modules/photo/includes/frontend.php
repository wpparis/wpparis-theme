<!-- add: "new windows" text notification on a link -->
<?php
...
<div class="fl-photo<?php if ( ! empty( $settings->crop ) ) echo ' fl-photo-crop-' . $settings->crop ; ?> fl-photo-align-<?php echo $settings->align; ?>" itemscope itemtype="http://schema.org/ImageObject">
	<div class="fl-photo-content fl-photo-img-<?php echo $filetype; ?>">
		<?php if(!empty($link)) : ?>
		<a href="<?php echo $link; ?>" target="<?php echo $settings->link_target; ?>" <?php if( $settings->link_title ): ?>title="<?php echo $settings->link_title; ?>"<?php elseif( '_blank' == $settings->link_target && trim( $alt ) != '' ): ?>title="<?php echo $alt .' - Nouvelle fenêtre'; ?>"<?php endif; ?> itemprop="url">
		<?php endif; ?>
